package com.github.dapeng.mockserver.web.util;
...
import com.github.dapeng.mockserver.web.vo.EventVo;
    ...
    public static Map<String, Collection<Service>> groupingServices(Collection<Service> services) {
        ...
        services.forEach(service -> {
            if (null != service.annotations) {
                service.annotations.stream().filter(x -> x.key.equals(Constants.SERVICE_GROUP_KEY))
                        .collect(Collectors.toList())
                        .forEach(att -> {
                            boolean existGroup = Constants.SERVICE_GROUP_KEY.equals(att.key);
                            if (existGroup) {
        ...
        if (tempServices.size() > 0) {
            groupedServices.put(Constants.SERVICE_GROUP_DEFAULT, tempServices);
        }
            ...
            service.methods.forEach(method -> {
                if (null != method.annotations) {
                    method.annotations.forEach(att -> {
                        boolean isVirtual = Constants.SERVICE_VIRTUAL_KEY
                                .equals(att.key) && Constants.SERVICE_VIRTUAL_VAL
    ...
    public static List<EventDto> findEventsByMethod(Method method) {
        ...
        if (null != method.getAnnotations()) {
            List<EventDto> eventDtos = method
                    .getAnnotations()
                    .stream()
                    .filter(x -> Constants.SERVICE_EVENT_KAY.equals(x.key))
                    .map(y -> new EventDto(
                            method.name,
                            y.value,
